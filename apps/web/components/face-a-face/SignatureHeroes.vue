...
<template>
  <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
    <UiComparisonColumn :title="meName" accent="brand">
      <div v-for="hero in me" :key="hero.heroId" class="flex items-center gap-3 rounded-lg border border-border bg-surface p-3">
        <HeroesHeroAvatar :hero-id="hero.heroId" :name="hero.heroName" :size="32" />
        <div class="min-w-0 flex-1">
          ...
          <div class="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted">
            <span :class="TONE_TEXT_CLASS[winrateTone(hero.winrate)]">{{ formatPercent(hero.winrate) }} winrate</span>
    ...
    <UiComparisonColumn :title="friendName" accent="accent">
      <div
