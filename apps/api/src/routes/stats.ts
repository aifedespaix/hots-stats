...
const summaryQuerySchema = z.object({ scope: heroStatsScopeSchema.optional(), mode: gameModeListSchema.optional() });
...
export const statsRoute = new Hono<Env>().use("*", authSession, requireUser).get("/summary", async (c) => {
  ...
  return c.json(await getStatsSummary(user.id, scope, parsed.data.mode));
});
