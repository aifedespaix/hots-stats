...
export const compareRoute = new Hono<Env>().use("*", authSession, requireUser).get("/:battletag", async (c) => {
  ...
  const [myOverview, myRoles, mySignature, oppOverview, oppRoles, oppSignature, synergy, matchups] =
    await Promise.all([
      getPlayerOverviewStats({ userId: user.id }),
      getRoleDistribution({ userId: user.id }),
      getSignatureHeroes({ userId: user.id }),
      getPlayerOverviewStats(target),
      getRoleDistribution(target),
      getSignatureHeroes(target),
      getSynergyStats(user.id, battletag),
      getMatchupStats(user.id, battletag),
    ]);
