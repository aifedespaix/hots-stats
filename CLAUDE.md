...
This project's knowledge graph is auto-maintained: a `post-commit` git hook rebuilds
`graphify-out/graph.json` after every commit (code changes only, no LLM needed). Before answering
a question about this codebase's architecture or file relationships, check `graphify-out/graph.json`
first — if it exists, run `graphify query "<question>"` rather than exploring from scratch. After a
change large enough to shift the module graph, mention that `/graphify --update` can refresh the
semantic (doc/image) layer too — the post-commit hook only covers code.
